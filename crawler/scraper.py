...
class Crawler(object):
    ...
    def get_car_offers(self, pages):
        ...
        for page in range(1, pages + 1):
            url = 'https://www.otomoto.pl/osobowe/poznan/?search%5Bbrand_program_id%5D%5B0%5D=&search%5Bdist%5D=50&search%5Bcountry%5D=&page={}'.format(page)
    ...
    def _get_offer_details(self, offer):
        url = "https://www.otomoto.pl/ajax/poznan/misc/contact/multi_phone/{}/0/".format(offer['dealer_phone_code'])
        response = self.session.get(url)
    ...
    def offer_exists(self, offer_id):
        ...
        # url = "https://www.otomoto.pl/osobowe/q-{}/?search%5Bbrand_program_id%5D%5B0%5D=&search%5Bcountry%5D=".format(offer_id)
        # response = self.session.get(url)
...
class Parser(object):
    ...
    def parse_car_offers(self, content):
        ...
        for item in items:
            ...
            phone = item.xpath('.//div[@class="seller-phones"]/div[@class="number-box"]/span[contains(@class, "spoiler")]')[0]
    ...
    def parse_car_offer(self, content):
        tree = html.fromstring(content)
        r = tree.xpath('//div[contains(@class, "offer-content__main-column")]//div[contains(@class, "seller-box__seller-info")]//h2[contains(@class, "seller-box__seller-name")]/descendant-or-self::*/text()')
        dealer_name = ''.join([c.strip() for c in r])

        r = tree.xpath('//div[contains(@class, "offer-params")]//li[contains(@class, "offer-params__item") and .//span[@class="offer-params__label" and text()="Marka"]]')[0]
        car_brand = r.xpath('.//div[@class="offer-params__value"]/a')[0].text.strip()

        r = tree.xpath('//div[contains(@class, "offer-params")]//li[contains(@class, "offer-params__item") and .//span[@class="offer-params__label" and text()="Model"]]')[0]
